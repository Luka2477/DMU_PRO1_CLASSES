package opgave4;

import java.util.ArrayList;
import java.util.Objects;

/**
 * Models a training plan for a Swimmer
 */
public class TrainingPlan {
	private char level;
	private int weeklyWaterHours;
	private int weeklyStrengthHours;

	private final ArrayList<Swimmer> swimmers = new ArrayList<>();
	
	public TrainingPlan(char level, int weeklyWaterHours, int weeklyStrengthHours) {
		this.level = level;
		this.weeklyWaterHours = weeklyWaterHours;
		this.weeklyStrengthHours = weeklyStrengthHours;
	}
	
	public char getLevel() {
		return level;
	}
	
	public void setLevel(char niveau) {
		this.level = niveau;
	}
	
	public int getWeeklyStrengthHours() {
		return weeklyStrengthHours;
	}
	
	public void setWeeklyStrengthHours(int weeklyStrengthHours) {
		this.weeklyStrengthHours = weeklyStrengthHours;
	}
	
	public int getWeeklyWaterHours() {
		return weeklyWaterHours;
	}
	
	public void setWeeklyWaterHours(int weeklyWaterHours) {
		this.weeklyWaterHours = weeklyWaterHours;
	}

	public void createSwimmer (String name, int yearGroup, ArrayList<Double> lapTimes, String club) {
		Swimmer swimmer = new Swimmer(name, yearGroup, lapTimes, club);
		this.swimmers.add(swimmer);
	}

	public void removeSwimmer (Swimmer swimmer) {
		this.swimmers.remove(swimmer);
	}

	public Swimmer getSwimmer (String name, String club) {
		for (Swimmer swimmer : this.swimmers) {
			if (swimmer.getName().equals(name) && swimmer.getClub().equals(club)) {
				return swimmer;
			}
		}

		return null;
	}

	public ArrayList<Swimmer> getSwimmers () {
		return new ArrayList<>(this.swimmers);
	}

	@Override
	public String toString() {
		StringBuilder result = new StringBuilder();
		for (Swimmer swimmer : this.getSwimmers()) {
			result.append(swimmer.getName()).append("'s bedste tid: ").append(swimmer.bestLapTime()).append("\n");
		}

		return result.toString();
	}
}
